...
<xsl:stylesheet
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template
                    ...
                    <xsl:with-param
                        name="string">WARNING: NO DENOMINATION FOUND</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template
        match="client_country">
        <xsl:param
            name="placeholderElement"
            select="/*/meta//client/country"/>
        <xsl:call-template
            name="checkPlaceholder">
            <xsl:with-param
                name="placeholderElement"
    ...
    <xsl:template
        match="company_country">
        <xsl:param
            name="placeholderElement"
            select="/*/meta/company/country"/>
        <xsl:call-template
            name="checkPlaceholder">
            <xsl:with-param
                name="placeholderElement"
    ...
    <xsl:template
        ...
        <xsl:value-of
            select="my:calculatePeriod($endDate, $startDate)"/>
    </xsl:template>
    <xsl:template
        match="contract_total_fee">
        <xsl:call-template
            name="getDenomination"/>
        <xsl:text>&#160;</xsl:text>
        <xsl:value-of
    ...
    <xsl:template
        match="contract_activities">
        <xsl:choose>
            <xsl:when
                test="/contract/meta/work/activities/activity">
                <xsl:call-template
            ...
            <xsl:otherwise>
                <xsl:call-template
                    name="displayErrorText">
                    <xsl:with-param
                        name="string">WARNING: NO ACTIVITIES FOUND</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    <xsl:template
        match="contractor_name_company">
        <xsl:param
            name="placeholderElement"
        ...
        <xsl:if
            test="/contract/meta/contractor/ctcompany">
            <xsl:text> (</xsl:text>
            <xsl:call-template
                name="checkPlaceholder">
                <xsl:with-param
    ...
    <xsl:template
        match="contractor_possessive_pronoun">
        <xsl:param
            name="placeholderElement"
    ...
    <xsl:template
        match="contractor_subject_pronoun">
        <xsl:param
            name="placeholderElement"
            select="/contract/meta/contractor/@sex"/>
        <xsl:param
    ...
    <xsl:template
        match="contractor_object_pronoun">
        <xsl:param
            name="placeholderElement"
            select="/contract/meta/contractor/@sex"/>
        <xsl:param
    ...
    <xsl:template
        match="generate_raterevisiondate">
        <xsl:param
            name="placeholderElement"
                        ...
                        <xsl:choose>
                            <xsl:when
                                ...
                                <xsl:for-each-group
                                    ...
                                    <xsl:if
                                        test="position() = last() and not(position() = 1)">
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when
                        test="$statusSequence">
                        <xsl:value-of
                            select="count(//finding[@status = $statusSequence])"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of
                            select="count(//finding)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template
        ...
        <xsl:choose>
            <xsl:when
                test="normalize-space($placeholderElement)">
                <!-- placeholder exists and contains text -->
                <xsl:choose>
                    <xsl:when
                        test="self::client_rate">
                        <xsl:call-template
                            name="getDenomination">
                            <xsl:with-param
                                name="placeholderElement"
                    ...
                    <xsl:when
                        ...
                        <xsl:call-template
                            name="displayErrorText">
                            <xsl:with-param
                                name="string">TBD</xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:when
                    ...
                    <xsl:when
                        test="self::contractor_possessive_pronoun">
                        <!-- some sexy logic -->
                        <xsl:choose>
                            <xsl:when
                                test="//contractor/@sex = 'M'">
                                <xsl:call-template
                                    name="getString">
                                    <xsl:with-param
                                        name="stringID"
                    ...
                    <xsl:when
                        test="self::contractor_subject_pronoun">
                        <!-- some sexy logic -->
                        <xsl:choose>
                            <xsl:when
                                test="//contractor/@sex = 'M'">
                    ...
                    <xsl:when
                        test="self::contractor_object_pronoun">
                        <!-- some sexy logic -->
                        <xsl:choose>
                            <xsl:when
                                test="//contractor/@sex = 'M'">
            ...
            <xsl:otherwise>
                <xsl:call-template
                    name="displayErrorText">
                    <xsl:with-param
                        name="string">[ WARNING: Cannot resolve placeholder <xsl:value-of
                            select="name()"/> in <xsl:value-of
                            select="base-uri()"/> - <xsl:call-template
                            name="getReason"><xsl:with-param
                                name="placeholderElement"
                                select="$placeholderElement"/></xsl:call-template> ]</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    <xsl:template
        ...
        <xsl:choose>
            <xsl:when
                test="/$placeholderElement"> element "<xsl:value-of
                    select="name($placeholderElement)"/>" (<xsl:call-template
                    name="getXPath"><xsl:with-param
                        name="element"
    ...
    <xsl:template
        ...
        <xsl:param
            ...
            select="."/>
        <xsl:for-each
            select="$element/ancestor-or-self::*">
            <xsl:value-of
